...
describe('transaction.model', () => {
  ...
  it('resolves category and account names for display', () => {
    ...
    expect(views[0]).toMatchObject({ categoryName: 'Alimentação', categoryIcon: 'restaurant', accountName: 'Conta corrente', accountType: 'BANK', destinationAccountName: null, displayStatus: 'PAID', ownerName: 'Alice' });
    expect(views[1]).toMatchObject({ categoryName: null, accountName: 'Conta corrente', destinationAccountName: 'Dinheiro' });
    expect(views[2]).toMatchObject({ categoryName: null, categoryIcon: null, accountName: '', accountType: null, ownerName: 'Alice', cardName: null });
