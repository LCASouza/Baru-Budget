...
Current state: v0.9 (Allocations and Settlements) — expenses split between people with the sum guaranteed by the database, settlements between users and derived receivable and payable balances, on top of the v0.8 recurrences, v0.7 installments, v0.6 cards, the dashboard, transactions, households and sharing, deployed at https://baru-budget.pages.dev. See `docs/PROJECT_STATUS.md` for the roadmap.
...
Local login: `supabase/seed.sql` creates the development users `dev@baru.local` and `dev2@baru.local` (password `baru-dev-123`), sample accounts, transactions and a credit card with invoices, installment purchases, recurring templates with the current month generated, a split expense with a partial settlement, a shared household and a VIEW grant from the second user to the first (local stack only; sign-ups are disabled).
...
- `docs/ARCHITECTURE_ANALYSIS_V0.1.md` to `V0.9.md` — approved architecture per version
